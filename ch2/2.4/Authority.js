...
class Authority extends Node {
  ...
  orderAndBroadcast(tx) {
    // give the transaction the latest nonce in the Authority node's state
    tx.contents.orderNonce = this.orderNonce;
    // increment the nonce
    this.orderNonce++;
    // sign the transaction to give it "proof of authority"
    const signedTx = EthCrypto.sign(this.wallet.privateKey, getTxHash(tx));
    // add the signed transaction to the history
    tx.sigs.push(signedTx);
