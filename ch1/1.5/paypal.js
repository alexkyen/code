...
class Paypal extends Client {
  ...
  stealAllFunds() {
    ...
    Object.keys(this.state).forEach((account) => {
  ...
  mintSecretFunds(amount) {
    ...
    this.generateTx(this.wallet.address, amount, 'mint');
  }
  ...
  checkUserAddress(tx) {
    ...
    const blacklisted = this.blacklist.find(address => tx.contents.from === address || tx.contents.to === address);
